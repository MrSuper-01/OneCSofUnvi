...
# YAOHENG1121 针对乳腺数据集进行修改；
# def load_video_and_mask_file(img_path: str,
#                              anno_path: str,
#                              frame_length: int = 10):
#     '''
#         Loads video and mask file for the breast dataset.
#         args:
#             img_path: videos .npy file path
#             anno_path: annotations .npz file path
...
#     # Collect masks:
#     mask_list = kpts_list['fnum_mask'].item()  # Load dictionary
#     idx_list = sorted([int(k) for k in mask_list.keys()])  # Convert keys to integers and sort them
#     masks = [mask_list[str(k)] for k in idx_list]  # Use sorted keys to retrieve masks

#     # Handle empty masks list
#     if len(masks) == 0:
#         raise ValueError(f"No masks found in the annotation file at {anno_path}")

#     # Ensure frame length doesn't exceed available frames
#     frame_length = min(frame_length, len(video))

#     # Compute step for selecting frames evenly
#     step = max(1, len(idx_list) // frame_length)

#     # Select frames indices
#     frame_inds = [idx_list[i] for i in range(0, len(idx_list), step)[:frame_length]]

#     # Collect frames and corresponding masks
#     frames = [video[i] for i in frame_inds]
#     selected_masks = [mask_list[str(i)] for i in frame_inds]

#     imgs = np.asarray(frames)
#     masks = np.asarray(selected_masks)

#     spacing = kpts_list['spacing']

#     return imgs, masks, spacing

# # 1127修改loader代码，由于之前不能正确计算；
def calculate_frame_positions(frame_count, total_frames):
    """
    ...
    """
    # 参数检查
    if frame_count <= 0 or total_frames <= 0:
        raise ValueError("frame_count 和 total_frames 必须是正整数")

    # 输出调试信息
    # print(f"Calculating frame positions with frame_count={frame_count}, total_frames={total_frames}")

    # 计算帧位置
    positions = [round(i * total_frames / frame_count) for i in range(frame_count)]
    # print(f"Raw positions before boundary correction: {positions}")

    # 修正边界，确保索引合法
    positions = [min(total_frames - 1, max(0, pos)) for pos in positions]
    # print(f"Corrected positions: {positions}")
...
def load_video_and_mask_file(img_path: str, anno_path: str, frame_length: int = 10):
    ...
    # Load video data
    video = np.load(img_path, allow_pickle=True)
    video = video.swapaxes(0, 1)  # Ensure shape is (F, C, H, W)
    total_frames = video.shape[0]  # Total number of frames in the video
    # print(f"Total frames in video: {total_frames}")

    # Load annotation data
    kpts_list = np.load(anno_path, allow_pickle=True)
    if 'fnum_mask' in kpts_list:
        mask_list = kpts_list['fnum_mask'].item()
        mask_keys = sorted([int(k) for k in mask_list.keys()])
        # print(f"Available mask keys: {mask_keys}") 核对一下
    else:
        raise ValueError(f"Key 'fnum_mask' not found in {anno_path}")

    if not mask_keys:
        raise ValueError(f"No masks found in annotation file: {anno_path}")

    # Calculate frame positions based on the given frame_length
    frame_positions = calculate_frame_positions(frame_length, total_frames)
    # print(f"Calculated frame positions: {frame_positions}")

    # Extract frames and corresponding masks
    frames = [video[pos] for pos in frame_positions]
    selected_masks = []
    for pos in frame_positions:
        if str(pos) in mask_list:
            selected_masks.append(mask_list[str(pos)])
        else:
            # print(f"Warning: No mask for frame {pos}. Using empty mask.")
            empty_mask = np.zeros_like(mask_list[str(mask_keys[0])])  # Use a template mask
            selected_masks.append(empty_mask)

    # # 数据完整性检查
    # if len(frames) != frame_length:
    #     print(f"Warning: Extracted {len(frames)} frames, but expected {frame_length}.")
    # if len(selected_masks) != frame_length:
    #     print(f"Warning: Extracted {len(selected_masks)} masks, but expected {frame_length}.")

    # Convert to numpy arrays
    imgs = np.asarray(frames)
    masks = np.asarray(selected_masks, dtype=np.float32)  # Preserve mask values
    spacing = kpts_list.get('spacing', None)  # Optional spacing information

    # 输出最终数据的形状信息
    # print(f"Final data shapes - imgs: {imgs.shape}, masks: {masks.shape}, spacing: {spacing}")
    # print(f"Video file path: {img_path}")
    # print(f"Annotation file path: {anno_path}")
    # print(f"Video file shape: {np.load(img_path, allow_pickle=True).shape}")
    # print(f"Annotation keys: {list(np.load(anno_path, allow_pickle=True).keys())}")
    return imgs, masks, spacing
...
class EchoVideoDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_size = img_size
        self.class_id = class_id
        self.class_dict_file = '/dahuafs/userdata/99309212/OtherProject/2025220/out_breast/class.json'  # 修改路径
        self.label_dict = {}
        with open("/dahuafs/userdata/99309212/OtherProject/2025220/out_breast/labels.txt", "r") as f:  # 打开文件
            for line in f.readlines():
                data = line.strip()  # 读取文件
                self.label_dict[data.split(' ')[0]] = int(data.split(' ')[1])

        dict_key_ls = list(self.label_dict.keys())
        random.shuffle(dict_key_ls)
        new_dic = {}
        for key in dict_key_ls:
            new_dic[key] = self.label_dict.get(key)
        self.label_dict = new_dic
    ...
    def __getitem__(self, i):
        ...
        for f in range(self.frame_length):
            class_label.append(self.label_dict[self.ids[i].split('.')[0]])
        ...
        # 调用修改后的 load_video_and_mask_file 函数
        image, mask, spacing = load_video_and_mask_file(
            img_path=os.path.join(img_path, prefix + '.npy'),
            anno_path=os.path.join(label_path, prefix + '_gt.npz'),
            frame_length=self.frame_length
        )
        # print(f"[DEBUG] Image shape: {image.shape}, Mask shape: {mask.shape}")
        # print(f"[DEBUG] Unique values in Mask: {np.unique(mask)}")
        # print(f"[DEBUG] Non-zero pixel count in Mask: {np.count_nonzero(mask)}")

        # 数据增强
        if self.joint_transform:
            # print("[DEBUG] Applying joint transform...")
        ...
        if not self.disable_point_prompt:
            ...
            # print(f"[DEBUG] Points generated: {pt.shape}, Labels: {point_label.shape}")

        # if self.one_hot_mask:
        #     assert self.one_hot_mask > 0, 'one_hot_mask must be nonnegative'
        #     mask = torch.zeros((self.one_hot_mask, mask.shape[1], mask.shape[2])).scatter_(0, mask.long(), 1)
        #     print(f"[DEBUG] One-hot encoded mask shape: {mask.shape}")

        # print(f"[DEBUG] Returning data for {filename}")
        return {
            'image': image,
            'label': mask,
            'p_label': point_labels,
...
#                  one_hot_mask: int = False,
#
#                  # frame_length: int = 2, # YAOHENG1121
#                  frame_length: int = 10,
#                  disable_point_prompt: bool = True,
#                  point_numbers: int = 1) -> None:
#         self.dataset_path = dataset_path
...
#         self.img_size = img_size
#         self.class_id = class_id
#         # self.class_dict_file = os.path.join(dataset_path, 'class.json')  1111
#         # self.class_dict_file = 'G:/WorkSpace/Medical_image_database/SupUtralVideo-main/MemSAM-main/MemSAM-main/EchoNet/echocycle/class.json'
#         self.class_dict_file = 'G:/WorkSpace/Medical_image_database/cardiac_data/breast/output_breast/class.json'  # YAOHENG修改1121
#
#         with open(self.class_dict_file, 'r') as load_f:
#             self.class_dict = json.load(load_f)
#         if joint_transform:
...
#         #     img_path=os.path.join(img_path, prefix + '.npy'),
#         #     # anno_path=os.path.join(label_path, prefix + '.npz'), # YAOHENG1121,
#         #     anno_path=os.path.join(label_path, prefix + '_gt.npz'),
#         #     frame_length=self.frame_length)
#         image, mask, spacing = load_video_and_mask_file(
#             img_path=os.path.join(img_path, prefix + '.npy'),
